...
const METRICS_ENDPOINTS = ["/api/admin/metrics/team", "/api/metrics/team", "/api/metrics"];
...
export default function AdminTeamMetricsPage() {
  ...
  async function fetchMetrics() {
    ...
    for (const base of METRICS_ENDPOINTS) {
      try {
    ...
    throw lastErr || new Error("Ne mogu da učitam metrike.");
  }
  ...
  function drawCharts() {
    ...
    const barData = w.google.visualization.arrayToDataTable([["Stage", "Opportunities"], ...rows]);
    const pieData = w.google.visualization.arrayToDataTable([["Stage", "Opportunities"], ...rows]);
    ...
    <main className="mx-auto max-w-6xl px-4 py-8">
      <div className="rounded-3xl border border-slate-200 bg-gradient-to-r from-cyan-50 via-fuchsia-50 to-yellow-50 p-6 shadow-sm">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-slate-900">Admin – Team metrics.</h1>
            <p className="mt-1 text-sm text-slate-700">Metrike celog sistema (admin scope) preko Google Charts grafika.</p>
          </div>
